...
@gestos_controller.route('/estadisticas/<string:tipo_gesto>/hoy', methods=['GET'])
def get_stats_today(tipo_gesto):
    """
    Endpoint para obtener estadísticas del día de hoy.
    """
    stats = gesto_service.get_stats_today(tipo_gesto)
    return jsonify(stats), 200

@gestos_controller.route('/estadisticas/<string:tipo_gesto>/ultimos7', methods=['GET'])
def get_stats_last_7(tipo_gesto):
    """
    Endpoint para obtener estadísticas de los últimos 7 días.
    """
    stats = gesto_service.get_stats_last_7_days(tipo_gesto)
    return jsonify(stats), 200
...
@gestos_controller.route('/estadisticas/<string:tipo_gesto>/historial', methods=['GET'])
def get_daily_history(tipo_gesto):
    """
    Endpoint para obtener el historial diario completo.
    Opcionalmente acepta parámetros de paginación.
    """
